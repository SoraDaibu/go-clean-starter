...
### 🔹 PostgreSQLドライバ: **[pgx](https://github.com/jackc/pgx)**

**pgx**をPostgreSQLドライバとして使用しています（stdlibモードではなくネイティブAPI）：

* **高いパフォーマンスと効率性**
  * 最適なパフォーマンスのためにテキストとバイナリの両方のワイヤプロトコルをサポート
  * 多くの操作で従来のdatabase/sqlドライバより2〜3倍高速
  * より効率的なメモリ使用とアロケーション削減
  * プリペアドステートメントキャッシュを備えたネイティブコネクションプーリング（`pgxpool`）

* **豊富なPostgreSQL機能サポート**
  * PostgreSQL固有の型（配列、JSONB、hstore、幾何学型など）のネイティブサポート
  * 高速な一括操作のための組み込みCOPYプロトコルサポート
  * リアルタイムイベント通知のためのLISTEN/NOTIFYサポート
  * カスタム型と複合型のより良いハンドリング

* **アクティブな開発とモダンな設計**
  * 定期的な更新とパフォーマンス改善で積極的にメンテナンスされている
  * モダンなGoパターンと最新のPostgreSQLバージョン向けに構築されている
  * コネクションプールメトリクスとヘルスチェックによる優れた可観測性
  * sqlcの`sql_package: "pgx/v5"`オプションを使用した場合の強い型安全性

* **本番環境対応のコネクション管理**
  * 自動ヘルスチェック付きの高度なコネクションプーリング
  * より良いリトライロジックとエラーハンドリング
  * コネクションライフサイクルフックとクエリロギングのサポート

pgxは`database/sql`ドライバ（stdlibモード）としても使用できますが、本プロジェクトでは、そのパフォーマンスとPostgreSQL固有の機能を最大限に活用するために**ネイティブpgx API**を使用しています。この決定は、本番環境で使用できる高性能で機能豊富なテンプレートを提供するという目標に合致しています。

---

### 🔹 ロギング: **[log/slog](https://pkg.go.dev/log/slog)（標準ライブラリ）**

構造化ロギングには`log/slog`を使用しています：

* **標準ライブラリ / 依存ゼロ**
  * Goに同梱されているため、ツールチェーンと共にバージョン管理・パッチ適用され、`go.mod`に何も追加しません。このテンプレートから派生したプロジェクトは、利用者がすでに慣れ親しんだ慣用的なロギングをそのまま継承できます。これは、このテンプレートが手動の依存性注入に対して適用しているのと同じ考え方です。

* **クリーンアーキテクチャとの親和性**
  * `slog`の`Handler`インターフェースは、呼び出し側のAPIと出力バックエンドを分離します。このテンプレートは`*slog.Logger`を小さな`Logger`インターフェース（`internal/logger`）でラップし、`builder`パッケージを通じて一度だけ注入します。各レイヤーは安定した契約に対してログを記録し、バックエンド（本番ではJSON、ローカルでは人間が読みやすいテキスト）は呼び出し側を変更することなく差し替えられます。
  * ログを記録するのはエントリーレイヤー（HTTPハンドラ／ミドルウェアとタスクレイヤー）のみです。そのため、サービスレイヤーやリポジトリレイヤーを伝播する過程で同じイベントが重複して記録されることはありません。`request_id`のようなリクエストスコープのフィールドはミドルウェアで導出され、リクエストの`context`を通じて伝播されます。

* **エコシステムとロードマップ**
  * `slog`はGoにおける構造化ロギングの事実上の標準インターフェースであり、OpenTelemetryのログ経路（将来の検討事項を参照）も`slog`を中心に構築されています。

**受け入れたトレードオフ:** `slog`の組み込みハンドラは、ゼロアロケーションのロガーと比べてアロケーションが多く、動作も遅くなります。とはいえ、スターターテンプレートのボトルネックはログのシリアライズではなくデータベースやネットワークのI/Oであるため、これは許容できます。ホットパスは利用者が`slog.LogAttrs`やカスタムハンドラで個別に最適化できます。
...
### **PostgreSQLドライバ: [lib/pq](https://github.com/lib/pq)**

`lib/pq`は事実上の標準PostgreSQLドライバとして検討されましたが、代わりに`pgx`を選択しました：

1. **アクティブなメンテナンスと将来性**
   * `lib/pq`は2018年以降メンテナンスモードで、新機能の開発はありません
   * `pgx`は定期的なパフォーマンス改善とバグ修正で積極的に開発されています
   * `pgx`は最新のPostgreSQL機能とGoバージョンをサポートしています

2. **パフォーマンス上の利点**
   * `pgx`はほとんどの操作で`lib/pq`より2〜3倍高速です
   * `pgx`のバイナリプロトコルサポートによりシリアライゼーションのオーバーヘッドが削減されます
   * `pgx`では、より効率的なメモリアロケーションとコネクションプーリングが提供されます

3. **PostgreSQL機能サポート**
   * `pgx`はPostgreSQLの配列、JSONB、カスタム型のネイティブサポートを提供します
   * `lib/pq`は多くのPostgreSQL固有の機能に対して手動マーシャリングが必要です
   * `pgx`はCOPYプロトコルやLISTEN/NOTIFYなどの高度な機能をサポートしています

4. **より良いツール統合**
   * `sqlc`は`sql_package: "pgx/v5"`でpgxをファーストクラスサポートしています
   * 型安全なコード生成がpgx型とシームレスに連携します
   * より優れた可観測性とデバッグ機能を提供します

`lib/pq`はシンプルなユースケースに適した安定したドライバですが、`pgx`は大幅に優れたパフォーマンス、豊富な機能、アクティブな開発を提供しており、モダンで本番環境対応のテンプレートにとってより良い選択となります。
...
### **ロギング: [zerolog](https://github.com/rs/zerolog)**

`zerolog`は当初のロガーでしたが、標準ライブラリの`log/slog`を優先して見直しました：

1. **外部依存 vs 標準ライブラリ**
   * `zerolog`は`go.mod`で追跡・更新が必要なサードパーティ依存です。`log/slog`はGoに同梱されているため、ツールチェーンと共にバージョン管理・パッチ適用され、派生プロジェクトに何も追加しません。

2. **エコシステムとの整合性**
   * `log/slog`はGoにおける構造化ロギングの事実上の標準インターフェースとなっており、OpenTelemetryのログ経路もこれを中心に構築されています。これに統一することで、派生するプロジェクトにとって慣用的なテンプレートを保てます。

3. **パフォーマンスのトレードオフ**
   * `zerolog`のゼロアロケーションな経路は確かに高速ですが、スターターテンプレートのボトルネックはログのシリアライズではなくデータベースやネットワークのI/Oです。`log/slog`の利便性と依存ゼロという性質は、生のスループット差を上回ります。また、ホットパスは`slog.LogAttrs`やカスタムハンドラで最適化できます。
