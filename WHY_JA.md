...
### 🔹 依存性注入: **[Wire](https://github.com/google/wire)**

**Wire**は依存性注入に使用され、オブジェクトの初期化をクリーンで型安全、かつ保守しやすく保つために選択されました：
* 手動配線を避け、特に依存関係が増加するにつれて依存関係を自動的にうまく配線します。
* コンストラクタを集中化し、明示的に保ちます。
* コンパイル時のコード生成を使用 — ランタイムリフレクションなし、オーバーヘッドゼロ。
* モジュラーでテスト可能なアーキテクチャを促進します（特にクリーンアーキテクチャのセットアップで有用）。

Wireは、サービス、リポジトリ、外部クライアントの数が増加しても、明確性を犠牲にすることなくプロジェクトをスケーラブルに保つのに役立ちます。
...
### **依存性注入ツール: [Fx](https://github.com/uber-go/fx)**

`Fx`は以下の理由で`Wire`を優先して見送られました：

1. **コンパイル時 vs ランタイムエラー**
   * `Wire`はコンパイル時にコードを生成するため、不一致や不足したコンストラクタは実行前に検出されます。
   * `Fx`はランタイムリフレクションに依存するため、DIエラーは`app.Start()`が呼び出されたときにのみ現れます。

2. **リフレクションオーバーヘッドなし**
   * `Wire`はコンストラクタを直接呼び出すプレーンなGoコードを生成 — 隠れた検索やパフォーマンスに悪影響はありません。
   * `Fx`は起動時にリフレクションを使用してグラフを構築するので、小さなオーバーヘッドがあります。

3. **中規模プロジェクトでのシンプルさ**
   * `Wire`は単一の`wire.Build(...)`呼び出しで配線を保持し、依存関係を明示的で追跡しやすくします。
   * `Fx`の`fx.Provide`/`fx.Invoke`モジュールとライフサイクルフックは、よりフレームワーク的な複雑さを導入します。
