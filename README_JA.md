...
- **Wire**: クリーンで型安全、保守可能なオブジェクト初期化のためのコンパイル時依存性注入
...
├── builder
│   ├── dependency.go
│   ├── wire.go         # 手で記述したwireコード
│   └── wire_gen.go     # `wire.go`から自動生成されたwireコード
├── cmd
...
### Docker
- `make build`を実行してDockerイメージをビルドします
```bash
make build
```

- `make up`を実行してdocker-composeでDockerコンテナを作成・実行します。
```bash
make up
```
